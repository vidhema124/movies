...
const Signup = () => {
  ...
  useEffect(() => {
    window.otpless = async (otplessUser) => {
      console.log("Received Otpless User:", otplessUser); // Debugging
      ...
      const identity = otplessUser.identities.find((id) => id.identityType === "MOBILE");
      if (!identity) {
        toast.error("Mobile identity not found!");
        return;
      }
      ...
      const userData = {
        ...
        ip: otplessUser.network?.ip || "Unknown",
        timezone: otplessUser.network?.timezone,
        ipLocation: otplessUser.network?.ipLocation || {},
      };
